### What is a Product?
A product is an individual piece of merchandise or service that can be offered in the market to satisfy customer's need. A product can be manufactured from the raw material or simply processed to be sold as finished good which is branded, packaged and labeled by the company. The products are distinguished based on the unique Ids/SKUs and are available with various features such as color, style, size, brands etc.
...
For eg: A T-shirt is a virtual product which is available in different sizes (small, medium, large and extra large) and colors (red, grey, blue, green and black). So Color and Size are the selectable features of Virtual product T-shirt which does not exist physically in the inventory. Now if I select Small size and Red color then actually there will be a variant T-shirt in which these two features are present as standard features.

**Product Association**
A Product Variant association (PRODUCT_VARIANT) is used to associate Virtual Product with its Variants. You can associate as many variants with a virtual product as feature combination is possible but only single unit of a variant will be associated with a virtual product.

For this association virtual product must be setup as a finished good and have isVirtual field set to 'Y' indicating that no inventory is kept for this product. Similarly the variant product must also be set as a finished good and have isVariant field set to 'Y'. The virtual products will have some selectable features associated with them that will be indeed associated as standard features with one or more variants.
...
Product packages, bundles and kits are collection or grouping of products which allows merchandiser to sell multiple products as a single unit. This facilitates the customer to buy multiple related products in one click. Components are the products which are collected together to be sold as a bundle or kit or package.

Bundles are collection of similar items allowing merchandiser to offer several units of a product for sale as one combined product. These are usually sold at low marginal cost therefore incites customer to purchase products in bulk. The components in the bundle can also be sold separately depending on the merchandising policies.
For eg: We often see chocolates packaged in a single packet say some 25 units and a single chocolate which can also be purchased separately.

Kits or Packages are collection of related items that are intended for same purpose or use. A merchandiser groups multiple related products to be sold in a kit or package that sometimes may not be sold independently. Kits or packages can be pre-built or customized depending on the merchandising requirement.

For example: A burger may be of $10, but it's combo package offering fries and a drink for $5 more. If purchased separately, the fries and drink would cost $5 each.
...
Product upgrades or Up-sell items are more advanced and expensive products or service which are offered by a merchandiser to the customer who is interested in buying another product.The upgraded products may have better quality, brand and of-course better features which customer might be interested in buying by just a little more investments.

The advantage of Up-sell products is that customers are often happier with the more advanced & expensive product while merchandiser earning more profitability from them.

For example: A merchandiser can offer a larger size LED television screen to the customers buying regular television screen or offer a combo of Set-top box attached with the television set.
...
For this association you should have products already setup as Finished Goods, which can be a standard, virtual or package product. A product can have any number of upgrade products associated with it. In case of large number of upgrade products they should be sequenced in the order to be displayed if merchandiser want to give priority to the products viewed by the customer at first glance.
...
It gives customer an idea of what else they can purchase that might be useful to them along with the product they originally purchased.
...
For this association you should already have the products setup as Finished Goods, which can be a standard, virtual or package product. A product can have any number of upgrade products associated with it. In case of large number of upgrade products they should be sequenced in the order to be displayed if merchandiser wants to give priority to the products viewed by the customer at first glance.
...
These products are quite similar to Cross-sell and Up-sell products but recommended products are not necessarily related based on price, feature or dependent usability. It can be any product which merchandiser wants the customer to know without having customer specifically to browse that product.

Product Association
Also Bought product association allow merchandiser to associate products that he/she would like to recommend to the customer with the current product being browsed or purchased. You can associate any number of products as Also-Bought or recommendation.
...
Purpose: This association is basically used by merchandiser to advertise or introduce customer with other products on the site without having customer browse that category/product by themselves.
